...
- **Campaign context summary**: a compact, wrapped row of small pill-style
  tags directly beneath the executive summary — confirming the submitted
  inputs the analysis actually used: Brand, Objective, Type, Occasion
  (only when set and not "None"), Audience (only when provided). Website
  is deliberately excluded — see `PRODUCT_SPEC.md`. Styled quieter than
  the summary above it (small, muted text, no card or border beyond the
  pills themselves) and spaced tightly against the executive summary
  rather than with the larger gap used between major sections, so it
  reads as confirmation of what was just said, not a new report section.
  Also included in "Copy summary" (see Actions below).
...
- **Strengths / Weaknesses**: two clearly separated lists beneath the
  context summary, each with a one-line description under its heading
...
- **Recommendations**: a numbered action list beneath strengths/weaknesses
  ...
  a location. Carries its own one-line description ("Concrete next steps
  to improve it.") and a small numbered index per item so it reads as a
  distinct, prescriptive action layer rather than a restyled copy of the
  Weaknesses list, even though several recommendations may originate from
  specific weaknesses. Deliberately not visually linked to a specific
  weakness — that correspondence isn't part of the report's data model
  and isn't guaranteed to hold once recommendations are generated by a
  real model instead of the mock engine.
- **Actions**: "Analyze another creative" (navigates back to `/analyze`,
  ...
  verdict, confidence, executive summary, campaign context, and
  recommendations — useful, cheap to build, no backend needed).
